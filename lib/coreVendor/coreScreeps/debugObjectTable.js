...
debugObjectTable.temp = function(tableObject, topLeftCornerData, toggleTitle) {

	var table = buildObjectTable(tableObject, topLeftCornerData, toggleTitle, "white");
	...
	for (var rowKey of rowKeys) {
		...
		for (var columnKey of columnKeys) {
			...
			if (tableObject[rowKey][columnKey]) {
				table += prettyString.toPrettyString(tableObject[rowKey][columnKey]);
			}
			table += "</td>";
		}
		table += "</tr>";
