					...
					if (creepsSpawnRule.remoteRooms) {
						...
						for (var remoteRoomCreepsSpawnRule of creepsSpawnRule.remoteRooms) {
							...
							if (remoteRoom) {
								...
								debug.danger(`No remote room name found for creepsSpawnRule `, remoteRoomCreepsSpawnRule);
							}
						}
					}
				}
	...
	if (!previousSpawnResult.waitForSpawn && !previousSpawnResult.spawned) {
		...
		if (creepMemory) {
			...
			if (isRemoteRoom) {
				...
				if (room.roomName !== spawn.room.roomName) {
...
creepsSpawner.scheduleOneTimeOneCreepRemoteRoomCreepsSpawnRule = function(spawnRoomName, remoteRoomCreepsSpawnRule, scheduleId) {
	...
	if (spawnRoomName && remoteRoomCreepsSpawnRule) {
		...
		if (!Memory.state.oneTimeOneCreepRemoteRoomCreepsSpawnRules[scheduleId]) {
			if (remoteRoomCreepsSpawnRule.spawnOrderMaxSpawnedCounts.length === 1) {

				if (!scheduleId) {
					scheduleId = Game.time.toString() + Math.random().toString();
				}
